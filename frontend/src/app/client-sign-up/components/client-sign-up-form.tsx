...
interface IClientSignUpFormProps {
  token: string;
}

const ClientSignUpForm = ({ token }: IClientSignUpFormProps) => {
  const router = useRouter();
  const { signUpClient, signUpDetails } = useClients();
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      firstName: signUpDetails?.client?.firstName,
      lastName: signUpDetails?.client?.lastName,
      email: signUpDetails?.client?.email,
      housenumberExtra: "",
    },
  });
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    signUpClient(
      token,
      values,
      router.push.bind(null, "/client-sign-up/congratulations")
    );
  }

  useEffect(() => {
    form.reset({
      ...signUpDetails?.client,
      housenumberExtra: "",
    });
  }, [signUpDetails]);
