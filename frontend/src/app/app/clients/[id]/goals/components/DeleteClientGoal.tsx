...
export default function DeleteClientGoalModal(props: IProps) {
  const { deleteClientGoal } = useClientGoals();
  if (!props.clientGoalId) return null;
  ...
  const handleDelete = async () => {
    ...
    await deleteClientGoal(props.clientGoalId as number, props.onClose);
  };
      ...
      <div className='w-full h-full flex justify-center z-50 absolute  top-0'>
        <div className='modal  w-fit bg-slate-600 border rounded-2xl dark:border-slate-800 dark:bg-slate-950 h-fit m-auto relative'>
          <Dialog.Panel className='mx-8 my-10 bg-inherit'>
            ...
            <span>Are you sure you want to delete this client goal?</span>
