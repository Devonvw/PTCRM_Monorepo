...
export class ClientGoalAchievementService {
  constructor(@InjectRepository(ClientGoalAchievement) private readonly clientGoalAchievementRepository: Repository<ClientGoalAchievement>, @InjectRepository(ClientGoal) private readonly clientGoalRepository: Repository<ClientGoal>, @InjectRepository(Client) private readonly clientRepository: Repository<Client>) { }
  async create(userId: number, body: CreateAchievementDto): Promise<any> {
    //. Make sure the provided client belongs to the coach (user)
    await this.checkIfClientBelongsToUserByClientGoalId(userId, body.clientGoalId);
  ...
  async update(userId: number, achievementId: number, body: UpdateAchievementDto): Promise<any> {
    //. Make sure the provided client belongs to the coach (user)
    var achievement: ClientGoalAchievement = await this.checkIfClientBelongsToUserByAchievementId(userId, achievementId);
    ...
    if (body.value <= 0) {
      throw new BadRequestException('The achievement value cannot be 0 or negative.');
    }
    ...
    if (achievement.clientGoal.completedValue <= body.value) {
      throw new BadRequestException(`The achievement value cannot be higher or equal to the client goal\'s completed value (which is ${achievement.clientGoal.completedValue}).`);
    }
    ...
    if (achievement.clientGoal.startValue >= body.value) {
      throw new BadRequestException(`The achievement value cannot be lower than or equal to the client goal\'s start value (which is ${achievement.clientGoal.startValue}).`);
    }
  ...
  async findAll(userId: number, query: GetAchievementsQueryDto): Promise<any> {
    ...
    if (query?.clientGoalId) {
      ...
      await this.checkIfClientBelongsToUserByClientGoalId(userId, query.clientGoalId);
    }
  ...
  private async checkIfClientBelongsToUserByAchievementId(userId: number, achievementId: number): Promise<ClientGoalAchievement> {
    //. Make sure the client belongs to the coach (user)
  ...
  private async checkIfClientBelongsToUserByClientId(userId: number, clientId: number): Promise<Client> {
    //. Make sure the client belongs to the coach (user)
