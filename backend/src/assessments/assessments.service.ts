...
export class AssessmentsService {
  constructor(@InjectRepository(Assessment) private assessmentRepository: Repository<Assessment>, @InjectRepository(ClientGoal) private clientGoalRepository: Repository<ClientGoal>, @Inject(ClientsService) private clientService, @Inject(ClientGoalsService) private clientGoalService) { }
  ...
  async create(userId: number, body: CreateAssessmentDto): Promise<any> {
    var achievementsObtained: ClientGoalAchievement[] = [];
    ...
    await this.assessmentRepository.manager.transaction(async entityManager => {
      ...
      for (const measurement of body.measurements) {
        const clientGoal: ClientGoal = await entityManager.findOne(ClientGoal, {
          where: {
      ...
      const savedAssessment = await entityManager.save(Assessment, assessment);


      //. Save the measurements to the database
      for (const measurement of body.measurements) {
        //. Create a new measurement object
        const measurementToSave = new Measurement({ ...measurement, assessment: { id: savedAssessment.id } as Assessment, clientGoal: { id: measurement.clientGoalId } as ClientGoal });
        ...
        //. If the goal has not been completed yet, check if any achievements have been made
          ...
          if (clientGoal.achievements.length > 0) {
            for (const achievement of clientGoal.achievements) {
              if (measurement.value >= achievement.value && !achievement.achieved) {
    ...
    if (achievementsObtained.length > 0) {
      return { message: 'Assessment saved', achievements: achievementsObtained };
    }
  ...
  async findAll(userId: number, query: GetAssessmentsQueryDto): Promise<any> {
    //. Make sure the client belongs to the coach (user)
    if (query.clientId) {
      const client = await this.clientService.getClientIfClientBelongsToUser(userId, query.clientId);
      if (!client) {
      ...
      const clientGoal = await this.clientGoalService.clientGoalExistsAndBelongsToUser(userId, query.clientGoalId);
      if (!clientGoal) {
        throw new NotFoundException('This client goal does not exist or does not belong to a client of yours.');
      }
  ...
  async findAllForClient(clientId: number, query: GetAssessmentsQueryDto): Promise<any> {
  ...
  async findAllForClientGoal(clientGoalId: number, query: GetAssessmentsQueryDto): Promise<any> {
    const pagination = Pagination(query);
    const orderBy = OrderBy(query, [
      {
        key: 'performedAt',
  ...
  async findOne(userId: number, assessmentId: number): Promise<any> {
    //. Make sure the client belongs to the coach (user)
    const assessment = await this.assessmentExistsAndBelongsToUser(assessmentId, userId);
  ...
  async update(userId: number, assessmentId: number, body: UpdateAssessmentDto): Promise<any> {
    //. Make sure the client belongs to the coach (user)
    const assessment = await this.assessmentExistsAndBelongsToUser(assessmentId, userId);
  ...
  async assessmentExistsAndBelongsToUser(assessmentId: number, userId: number): Promise<Assessment> {
